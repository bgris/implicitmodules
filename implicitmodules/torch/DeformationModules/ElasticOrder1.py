...
class ImplicitModule1(DeformationModule):
    ...
    def fill_controls_zero(self):
        self.fill_controls(torch.zeros(self.__dim_controls), device=self.__device)
    ...
    def compute_geodesic_control(self, man):
        ...
        S = torch.tensordot(S, eta(self.__device), dims=2)
    ...
    def __compute_aqh(self, h):
        ...
        return torch.einsum('nli, nik, k, nui, niv, lvt->nt', R, self.__C, h, torch.eye(self.__manifold.dim, device=self.__device).repeat(self.__manifold.nb_pts, 1, 1), torch.transpose(R, 1, 2), eta(self.__device))

    def __compute_sks(self):
        self.__sks = compute_sks(self.__manifold.gd[0].view(-1, self.__manifold.dim), self.sigma, 1) + self.__nu * torch.eye(3 * self.__manifold.nb_pts, device=self.__device)
    ...
    def __compute_moments(self):
        ...
        self.__moments = torch.tensordot(self.__lambdas.view(-1, 3), torch.transpose(eta(self.__device), 0, 2), dims=1)

    def __compute_aqkiaq(self):
        lambdas = torch.zeros(self.__dim_controls, 3 * self.__manifold.nb_pts, device=self.__device)
        aq = torch.zeros(3 * self.__manifold.nb_pts, self.__dim_controls, device=self.__device)
        for i in range(self.__dim_controls):
            h = torch.zeros(self.__dim_controls, device=self.__device)
            h[i] = 1.
