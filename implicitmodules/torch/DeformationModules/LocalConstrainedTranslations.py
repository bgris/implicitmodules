...
class LocalLinear(ConstrainedTranslationsBase):
    def __init__(self, manifold, A, sigma, coeff, label=None):
        ...
        def f_vectors(gd):
            direc = f_support(gd) - gd.repeat(3, 1)
            return torch.mm(direc, A.transpose(0,1))
        self.__A = A
        super().__init__(manifold, sigma, f_support, f_vectors, coeff, label)
   
    @classmethod
    def build(cls, dim, A, sigma, coeff, gd=None, tan=None, cotan=None, label=None):
        """Builds the Translations deformation module from tensors."""
        return cls(Landmarks(dim, 1, gd=gd, tan=tan, cotan=cotan), A, sigma, coeff, label)
