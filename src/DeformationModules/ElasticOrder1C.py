# -*- coding: utf-8 -*-
"""
Created on Fri Jan 30 11:54:39 2019
...
import src.GeometricalDescriptors.x_R_C as GeoDescr
...
class ElasticOrder1C(ab.DeformationModule):
    """
     Elastic module of order 1 where C in in the GD
    """
    
    
    def __init__(self, sigma, N_pts, dim, coeff, nu, dimCont):
        """
        ...
        self.nu = nu
        self.dimR = 3
        self.GD = GeoDescr.GD_xR_C(N_pts, dim, dimCont)
    ...
    def copy(self):
        return ElasticOrder1C(self.sig, self.N_pts, self.dim, self.coeff, self.nu, self.dimcont)
    
    def copy_full(self):
        Mod = ElasticOrder1C(self.sig, self.N_pts, self.dim, self.coeff, self.nu, self.dimcont)
    ...
    def DerCost_curr(self):
        ...
        cotR = 2*np.asarray([np.dot(np.dot(tP[i],R[i]),np.diag(np.dot(C[i],self.Cont)))
                             for i in range(x.shape[0])])

        # for each i, matrix multiplication of Mom_i and R_i
        Mom_R = np.einsum('ijk,ikl->ijl', self.Mom,R)   
        # for each i, diagonal of marix mult R_i^T Mom_i R_i
        RT_Mom_R = np.einsum('ijk,ikj->ij', np.transpose(R, [0,2,1]),Mom_R)
        # for each i, matrix mult of Cont and diagonal of marix mult R_i^T Mom_i R_i
        hRT_Mom_R = self.coeff *np.einsum('j,ik->ikj',self.Cont,RT_Mom_R)                    
                             
                             
        out.cotan = (cotx, cotR, hRT_Mom_R)
    ...
    def cot_to_innerprod_curr(self, GDCot, j):#
        ...
        cotx0, cotR0, cotC0 = der0.cotan
        
        # for each i, matrix multiplication of Mom_i and R_i
        Mom_R = np.einsum('ijk,ikl->ijl', tP, R)   
        # for each i, diagonal of marix mult R_i^T Mom_i R_i
        RT_Mom_R = np.einsum('ijk,ikj->ij', np.transpose(R, [0,2,1]),Mom_R)
        # for each i, matrix mult of Cont and diagonal of marix mult R_i^T Mom_i R_i
        hRT_Mom_R = np.einsum('j,ik->ikj',self.Cont,RT_Mom_R)                    



        out.cotan = (cotx0.copy(), cotR.copy(), hRT_Mom_R.copy())
    ...
    def p_Ximv_curr(self, vs, j):
        ...
        if j==0:
            ...
            out.cotan = (der.copy(), np.zeros([self.N_pts, self.dim, self.dim]), np.zeros([self.N_pts, self.dim, self.dimcont]))
